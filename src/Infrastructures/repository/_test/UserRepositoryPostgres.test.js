...
describe("UserRepositoryPostgres", () => {
    ...
    describe("getUserById", () => {
        it("should throw InvariantError when user not found", async () => {
            ...
            // Action & Assert
            await expect(
                userRepositoryPostgres.getUserById("notfound user id")
            ).rejects.toThrowError(InvariantError);
        });
        ...
        it("should return user detail correctly", async () => {
            ...
            const userPayload = {
                id: "user-321",
                username: "dicodinglagi",
                fullname: "ini nama lengkap",
            };
            await UsersTableTestHelper.addUser(userPayload);
            const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

            // Action
            const user = await userRepositoryPostgres.getUserById(
                userPayload.id
            );

            // Assert
            expect(user.username).toEqual(userPayload.username);
            expect(user.fullname).toEqual(userPayload.fullname);
        });
    });
});
